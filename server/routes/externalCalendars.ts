...
export async function externalCalendarRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/external-calendars", async () => listExternalCalendarSources());
  ...
  app.get<{ Querystring: { from?: string; to?: string } }>("/api/external-calendar-events", async (request, reply) => {

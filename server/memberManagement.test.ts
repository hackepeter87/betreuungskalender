...
test("explicit installation owner can update another member role", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(invited.id, persistence.query), "editor");
...
test("non-owners cannot administer members when an owner is configured", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await canAdministerMembers(otherAdmin, persistence.query), false);
...
test("admin fallback can administer members before an explicit owner exists", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await canAdministerMembers(admin, persistence.query), true);
...
test("users cannot change their own role through member management", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(owner.id, persistence.query), undefined);
  });
});
...
test("member listing exposes claim and effective roles without tokens", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    const members = await listMembers(persistence.query);
...
test("deployed user listings omit the exact local development identity", async () => {
  await withDatabase(async (_database, persistence) => {
    ...
    assert.equal(
      (await listMembers(persistence.query, deployedOptions)).some((member) => member.id === "local-dev"),
      false
    );
    assert.equal(
      (await listAppUsers(persistence.query, deployedOptions)).some((member) => member.id === "local-dev"),
      false
    );

    assert.equal(
      (await listMembers(persistence.query, { includeLocalDevelopmentIdentity: true }))
        .some((member) => member.id === "local-dev"),
      true
    );
    assert.equal(
      (await listAppUsers(persistence.query, { includeLocalDevelopmentIdentity: true }))
        .some((member) => member.id === "local-dev"),
      true
    );
  });

...
| `APP_RELEASE_VERSION` | Release Compose image tag | `1.4.3` | Required for `deploy/compose.yml` | None | Must match the extracted release package version |
| `APP_RELEASE_DIR` | Release Compose build context | `/opt/svc_betreuung/betreuungskalender/releases/v1.4.3` | Required for `deploy/compose.yml` | None | Must point at the verified extracted release directory |

...
test("valid invitations can be accepted by authenticated users", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(user.id, persistence), "editor");
  });
});
...
test("transfer-linked invitations apply only the selected role and proposed care parties", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(user.id, persistence), "scheduler");
...
test("expired invitations cannot be accepted", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(user.id, persistence), undefined);
  });
});
...
test("accepted invitations cannot be reused", async () => {
  await withDatabase(async (database, persistence) => {
    ...
    assert.equal(await membershipRoleForUser(firstUser.id, persistence), "editor");
    assert.equal(await membershipRoleForUser(secondUser.id, persistence), undefined);
  });
});

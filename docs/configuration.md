...
Audit records created through the API use `local-dev` as identity. That
technical identity is available only in local mode. Deployed authentication
modes do not expose it as a workspace member or assignment candidate, while
historical audit references remain readable. Starts that intentionally run
without authentication must set `AUTH_MODE=local` explicitly; the default
production examples keep `REQUIRE_AUTH=true` so an accidental direct deployment
fails closed.

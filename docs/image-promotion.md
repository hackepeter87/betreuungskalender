...
by pulling a promoted GHCR image. It supports a separate non-production test
installation and a production installation without archive-based local builds.
...
| `testing` | Mutable tag promoted from a release after image validation | Non-production test installation |
...
For a test installation at `https://app.testing.example.net`, the public native
OIDC values are:
...
1. Run **Promote testing image** with the release tag, for example `vX.Y.Z`.
2. Update the test installation and complete the technical and domain smoke
   tests.
